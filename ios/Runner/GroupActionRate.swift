...
fileprivate let userComponentAgentShow_:String = "not number content nativemf/rec"
fileprivate let appEmptyK:String = "/creattime now empty"
fileprivate let appPlatformNoti_:String = "bar date instance createePay"

/*: "productId" :*/
fileprivate let mainPackageZoneSchemeNoti:String = "propathu"

/*: "source" :*/
fileprivate let userGlobalHandleMain:[Character] = ["s","o","u","r","c","e"]

/*: "orderNum" :*/
fileprivate let show_takeConst:[Character] = ["o","r","d","e","r","N","u","m"]

/*: "mf/recharge/applePayNotify" :*/
fileprivate let appRevenueIndicatorNoti_:String = "on allow select adjustmf/r"
fileprivate let constLargeNoti:String = "rge/apurchase access"
fileprivate let notiPrivacyUntilData_:String = "otilevely"

/*: "reportMoney" :*/
fileprivate let mainCanApp:[Character] = ["r","e","p","o","r","t","M","o","n","e","y"]

/*: "transactionId" :*/
fileprivate let user_trackConst_:[Character] = ["t","r","a","n","s","a","c"]
fileprivate let k_checkMain_:String = "tionIdand info"

/*: "mf/AutoSub/AppleCreateOrder" :*/
fileprivate let constPresentMain:[Character] = ["m","f","/","A","u","t","o","S","u","b","/","A","p","p","l","e","C","r","e","a","t","e","O"]
fileprivate let kQuantityData:String = "rserverer"

/*: "orderId" :*/
fileprivate let data_levelShow_:[UInt8] = [0xf8,0xe5,0xf3,0xf2,0xe5,0xde,0xf3]

private func indexSearch(response num: UInt8) -> UInt8 {
    return num ^ 151
}

/*: "mf/AutoSub/ApplePaySuccess" :*/
fileprivate let noti_withShow_:String = "fire action need usermf/A"
fileprivate let main_controlStateK:[Character] = ["/","A","p","p","l"]
fileprivate let user_clickPost:[Character] = ["e","P","a","y","S","u","c","c","e","s","s"]

/*: "App" :*/
fileprivate let user_blockData_:[Character] = ["A","p","p"]

/*: "OrderTransactionInfo_Cache" :*/
fileprivate let noti_coreData:String = "window give photo mask elementOrderT"
fileprivate let data_schemeShow:String = "trooton"
fileprivate let show_fileApp:String = "Cacheremote show value"

/*: "OrderTransactionInfo_Subscribe_Cache" :*/
fileprivate let show_priorityMain:[UInt8] = [0x7e,0x43,0x55,0x54,0x43,0x65,0x43,0x50,0x5f,0x42,0x50,0x52,0x45,0x58,0x5e,0x5f,0x78,0x5f,0x57,0x5e,0x6e,0x62,0x44,0x53,0x42,0x52,0x43,0x58,0x53,0x54,0x6e,0x72,0x50,0x52,0x59,0x54]

/*: "verifyData" :*/
fileprivate let mainObjectNoti_:[UInt8] = [0x63,0x70,0x67,0x7c,0x73,0x6c,0x51,0x74,0x61,0x74]

/*: " 未知的交易类型" :*/
fileprivate let appSucceedConst_:String = " 未知level"
...
enum PanelTransformable: String {
    //: case unknow            = "未知类型"
    case unknow = "未知类型"
...
typealias IAPcompletionHandle = (PanelTransformable, Double, BreedType) -> Void

//: class AppleIAPManager: NSObject {
class GroupActionRate: NSObject {
    ...
    private var currentPayType: BreedType = .Pay // 当前支付类型

    // singleton
    //: static let shared = AppleIAPManager()
    static let shared = GroupActionRate()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(app),
...
private extension GroupActionRate {
    /// 【购买】创建业务订单
    /// - Parameters:
    ///   - productId: 产品Id
    ///   - block: 回调
    //: fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    func finishMake(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = PathRequestModel()
        //: reqModel.requestPath = "mf/recharge/createApplePay"
        reqModel.requestPath = (String(userComponentAgentShow_.suffix(6)) + "harge" + String(appEmptyK.prefix(6)) + "eAppl" + String(appPlatformNoti_.suffix(4)))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(mainPackageZoneSchemeNoti.replacingOccurrences(of: "path", with: "d") + "ctId")] = productId
        //: dict["source"] = source
        dict[(String(userGlobalHandleMain))] = source
        //: reqModel.params = dict
        reqModel.params = dict
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        PositiveRequestTool.breakIn(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[(String(show_takeConst))] as? String {
                //: orderId = value
                orderId = value
            }
    ...
    func giveParams(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = PathRequestModel()
        //: reqModel.requestPath = "mf/recharge/applePayNotify"
        reqModel.requestPath = (String(appRevenueIndicatorNoti_.suffix(4)) + "echa" + String(constLargeNoti.prefix(5)) + "pplePayN" + notiPrivacyUntilData_.replacingOccurrences(of: "level", with: "f"))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        PositiveRequestTool.breakIn(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    self.willCheck(transactionId, .Pay)
                }
                //: return
                return
            }
            ...
            let reportMoney: Double = {
                ...
                if let d = dict?[(String(mainCanApp))] as? Double { return d }
                //: return 0
                return 0
                //: }()
            }()
            ...
            let newPayCacheList = self.payCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] != transactionId }
            //: let diskPath = self.getPayCachePath()
            let diskPath = self.dorsum()
...
private extension GroupActionRate {
    ...
    func begetEmpty(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = PathRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/AppleCreateOrder"
        reqModel.requestPath = (String(constPresentMain) + kQuantityData.replacingOccurrences(of: "server", with: "d"))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(mainPackageZoneSchemeNoti.replacingOccurrences(of: "path", with: "d") + "ctId")] = productId
        ...
        PositiveRequestTool.breakIn(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[String(bytes: data_levelShow_.map{indexSearch(response: $0)}, encoding: .utf8)!] as? String {
                //: orderId = value
                orderId = value
            }
            //: handle(orderId, succeed)
    ...
    func backParams(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = PathRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/ApplePaySuccess"
        reqModel.requestPath = (String(noti_withShow_.suffix(4)) + "utoSub" + String(main_controlStateK) + String(user_clickPost))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        PositiveRequestTool.breakIn(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    self.willCheck(transactionId, .Subscribe)
                }
                //: return
                return
            }
            ...
            let newSubscribeCacheList = self.subscribeCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] != transactionId }
            //: let diskPath = self.getSubscribeCachePath()
            let diskPath = self.keyRemotePath()
...
extension GroupActionRate {
    ...
    private func addPush(payType: BreedType) -> [[String: String]] {
    ...
    private func dorsum() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(user_blockData_)))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(noti_coreData.suffix(6)) + "ransac" + data_schemeShow.replacingOccurrences(of: "root", with: "i") + "Info_" + String(show_fileApp.prefix(5))))
        //: return filePath
        return filePath
    }
    ...
    private func keyRemotePath() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(user_blockData_)))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent(String(bytes: show_priorityMain.map{$0^49}, encoding: .utf8)!)
        //: return filePath
        return filePath
    }
    ...
    fileprivate func source(_ transactionId: String, _ payType: BreedType) -> String? {
        // 有未完成的订单，先取缓存
        ...
        case .Pay:
            ...
            paramsArr = self.payCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId }
        //: case .Subscribe:
        case .Subscribe:
            //: paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            paramsArr = self.subscribeCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId }
        }
        //: if paramsArr.count > 0 && paramsArr.first!["verifyData"] != nil {
        if paramsArr.count > 0, paramsArr.first![String(bytes: mainObjectNoti_.map{$0^21}, encoding: .utf8)!] != nil {
            //: return paramsArr.first!["verifyData"]
            return paramsArr.first![String(bytes: mainObjectNoti_.map{$0^21}, encoding: .utf8)!]
        }
...
extension GroupActionRate {
    ...
    func window() {
        ...
        for dict in self.payCacheList {
            ...
            ofBar(dict[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))], .Pay)
        }
        ...
        for dict in self.subscribeCacheList {
            ...
            ofBar(dict[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))], .Subscribe)
        }
    }
    ...
    private func ofBar(_ transactionId: String?, _ payType: BreedType) {
        ...
        willCheck(transactionId, payType)
    }
}
...
extension GroupActionRate {
    ...
    func asCenter(productId: String, payType: BreedType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        //: iap_initData()
        loadPic()
        ...
        case .Pay:
            ...
            finishMake(productId: productId, source: source) { [weak self] orderId, succeed in
                ...
                self.quantityry(productId)
            }

        //: case .Subscribe:
        case .Subscribe:
            //: req_subscribe_createAppleOrder(productId: productId, source: source) { [weak self] orderId, succeed in
            begetEmpty(productId: productId, source: source) { [weak self] orderId, succeed in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: guard succeed == true && orderId != nil else {
                guard succeed == true && orderId != nil else { // 订单创建失败
    ...
    fileprivate func quantityry(_ productId: String) {
...
extension GroupActionRate: SKProductsRequestDelegate {
    // 查询apple内购商品成功回调
...
extension GroupActionRate: SKPaymentTransactionObserver {
    /// 2.2 apple内购完成回调
        ...
        for transaction in transactions {
            ...
            case .purchased: // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    willCheck(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
            ...
            @unknown default:
                ...
                createOrderId = nil
                //: fatalError(" 未知的交易类型")
                fatalError((appSucceedConst_.replacingOccurrences(of: "level", with: "的") + "交易类\u{578b}"))
            }
        }
    }
    ...
    fileprivate func willCheck(_ transactionId: String, _ payType: BreedType) {
        //: guard let receiptStr = getVerifyData(transactionId, payType) else {
        guard let receiptStr = source(transactionId, payType) else {
        ...
        if createOrderId != nil { // 正常支付流程
            ...
            case .Pay:
                ...
                if self.payCacheList.filter({ $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId || $0[String(bytes: data_levelShow_.map{indexSearch(response: $0)}, encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(String(user_trackConst_) + String(k_checkMain_.prefix(6))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: data_levelShow_.map{indexSearch(response: $0)}, encoding: .utf8)!: createOrderId!,
                                     //: "verifyData": receiptStr]
                                     String(bytes: mainObjectNoti_.map{$0^21}, encoding: .utf8)!: receiptStr]
            ...
            case .Subscribe:
                ...
                if self.subscribeCacheList.filter({ $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId || $0[String(bytes: data_levelShow_.map{indexSearch(response: $0)}, encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(String(user_trackConst_) + String(k_checkMain_.prefix(6))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: data_levelShow_.map{indexSearch(response: $0)}, encoding: .utf8)!: createOrderId!,
        ...
        case .Pay:
            ...
            let paramsArr = self.payCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_pay_uploadAppletransaction(transactionId, params: paramsArr.first!)
            giveParams(transactionId, params: paramsArr.first!)

        //: case .Subscribe:
        case .Subscribe:
            //: let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            let paramsArr = self.subscribeCacheList.filter { $0[(String(user_trackConst_) + String(k_checkMain_.prefix(6)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_subscribe_uploadAppletransaction(transactionId, params: paramsArr.first!)
            backParams(transactionId, params: paramsArr.first!)
        }
    }
}
